...
- 読書位置は自動保存され、書架上部の「続きから」で最後に開いた作品の続きへ戻れます。読書画面の下部には字数と、現在位置からの残り時間(読み終えると「読了」)が出ます
- 書架の作品が5冊を超えると、上部に絞り込み欄が出て題名・著者で書架を検索できます
...
- `src/lib/reading.ts` — 字数・推定読書時間・残り時間・書架の絞り込み・しおりの抜き出し・検索の素材づくり(純粋関数)
